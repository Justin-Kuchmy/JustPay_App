...
std::string PayrollRepository::getCreateTableSQL() const
{
    return R"(
            ...
            pay_period_label   VARCHAR(50)  NOT NULL, -- "August 1-15, 2025"
            pay_period_number  INTEGER      NOT NULL CHECK (pay_period_number IN (1, 2)),
        ...
        INSERT INTO payroll_records (employee_id, full_name, department, pay_period_label, pay_period_number, basic_salary, allowances, overtime_pay, adjustments, gross_income, sss_premium, philhealth_premium, hdmf_premium, loan_deductions, withholding_tax, total_deductions, net_pay ) 
        VALUES
        ('00-0001','Alice Santos','HR','August 1-15, 2025',1,22500.00,2500.00,0.00,0.00,25000.00,1125.00,0.00,0.00,2083.33,1879.10,5087.43,19912.57),
        ('01-0002','Bob Reyes','Finance','August 1-15, 2025',1,14000.00,1000.00,0.00,0.00,15000.00,700.00,0.00,0.00,5000.00,432.44,6132.44,8867.56),
        ('01-0003','Mary Mabulay','Finance','August 1-15, 2025',1,40000.00,2500.00,1729.29,0.00,44229.29,1750.00,0.00,0.00,2500.00,5932.27,10182.27,34047.02),
        ('02-0004','Carlos Dela Cruz','IT','August 1-15, 2025',1,27500.00,1500.00,0.00,0.00,29000.00,1375.00,0.00,0.00,0.00,2829.10,4204.10,24795.90),
        ('02-0005','Janine Uy','IT','August 1-15, 2025',1,21000.00,1250.00,67.71,0.00,22317.71,1050.00,0.00,0.00,0.00,1607.64,2657.64,19660.07),
        ('03-0006','Patrick Gomez','Operations','August 1-15, 2025',1,19000.00,750.00,0.00,0.00,19750.00,950.00,0.00,0.00,0.00,1214.09,2164.09,17585.91),
        ('03-0007','Lea Navarro','Operations','August 1-15, 2025',1,32500.00,2500.00,0.00,0.00,35000.00,1625.00,0.00,0.00,0.00,3779.10,5404.10,29595.90),
        ('04-0008','Jasmine Co','Sales','August 1-15, 2025',1,12500.00,600.00,53.74,0.00,13153.74,625.00,0.00,0.00,0.00,226.75,851.75,12301.99),
        ('04-0009','Miguel Tan','Sales','August 1-15, 2025',1,23500.00,1500.00,0.00,0.00,25000.00,1175.00,0.00,0.00,0.00,2069.10,3244.10,21755.90),
        ('05-0010','Katrina Ramos','Marketing','August 1-15, 2025',1,15000.00,750.00,0.00,0.00,15750.00,750.00,0.00,0.00,0.00,574.94,1324.94,14425.06);
...
PayrollCalculationResults PayrollRepository::mapPayroll(sqlite3_stmt *stmt)
{
    ...
    const unsigned char *payrollPeriod = sqlite3_column_text(stmt, 1);
    payrollRecord.payrollPeriod = payrollPeriod ? reinterpret_cast<const char *>(payrollPeriod) : std::string{};
    ...
    const unsigned char *employeeId = sqlite3_column_text(stmt, 3);
    ...
    const unsigned char *employeeDepartment = sqlite3_column_text(stmt, 5);
    ...
    payrollRecord.deductionFirstHalf = sqlite3_column_int(stmt, 15);
    payrollRecord.deductionSecondHalf = sqlite3_column_int(stmt, 16);
    payrollRecord.withHoldingTax = sqlite3_column_int(stmt, 17);
    payrollRecord.totalDeductions = sqlite3_column_int(stmt, 18);
    payrollRecord.netPay = sqlite3_column_int(stmt, 19);
...
sqlite3_int64 PayrollRepository::insertPayroll(const PayrollCalculationResults &prRecord)
{
    ...
    // derive pay period number from payrollPeriod string
    int payPeriodNumber = (prRecord.payrollPeriod == "first") ? 1 : 2;
    ...
    sqlite3_bind_text(stmt, idx++, prRecord.payrollPeriod.c_str(), -1, SQLITE_TRANSIENT);
...
{
    std::string sql = std::format(
        "UPDATE payroll_records SET employee_id = '{}', full_name = '{}', department = '{}', pay_period_label = '{}', pay_period_number = {}, basic_salary = {}, allowances = {}, overtime_pay = {}, adjustments = {}, gross_income = {}, sss_premium = {}, philhealth_premium = {}, hdmf_premium = {}, loan_deductions = {}, withholding_tax = {}, total_deductions = {}, net_pay = {} WHERE id = {}",
        prRecord.employeeId,
        prRecord.fullName,
        prRecord.employeeDepartment,
        prRecord.payrollPeriod,
        (prRecord.payrollPeriod == "first" ? 1 : 2),

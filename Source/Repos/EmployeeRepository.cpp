...
std::string EmployeeRepository::insertEmployee(const Employee& employee)
{
    ...
    sqlite3_bind_text(stmt, 13, employee.clockInTimeStr.c_str(), -1, SQLITE_TRANSIENT);    
    sqlite3_bind_text(stmt, 14, employee.clockOutTimeStr.c_str(), -1, SQLITE_TRANSIENT);    
    sqlite3_bind_double(stmt, 15, employee.monthlyBasicSalary);    
    sqlite3_bind_double(stmt, 16, employee.monthlyAllowances);    
    sqlite3_bind_text(stmt, 17, employee.personalEmail.c_str(), -1, SQLITE_TRANSIENT);    
    sqlite3_bind_text(stmt, 18, employee.personalMobileNumber.c_str(), -1, SQLITE_TRANSIENT);    
    sqlite3_bind_int(stmt, 19, employee.isActive);  

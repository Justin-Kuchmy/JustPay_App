...
void PremiumValuesWidget::applyValues()
{
    for (size_t i{}; i < dataBus->size(); i++)
    {
        ...
        emp.payPeriodDate = emp.dateProcessed.toMonthYearString();
        ...
        // premium values are only applicable on certain payperiods, prService.applies returns true or false based on deduction schedule.
        emp.applySSS = prService.applies(config.sssSchedule, emp.payPeriodHalf);
        emp.sssPremium_EE = PayrollCalc::truncateForCurrency(PayrollCalc::calcSSS_EE(emp.monthlyBasicSalary, emp.applySSS));

        emp.applyPhilHealth = prService.applies(config.philHealthSchedule, emp.payPeriodHalf);
        ...
        emp.applyHDMF = prService.applies(config.hdmfSchedule, emp.payPeriodHalf);

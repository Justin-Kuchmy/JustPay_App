                                                                ...
                                                                optAttendanceLogs{}
{
    ...
    connect(ui->addButton, &QPushButton::clicked, this, &EmployeeDetailsWidget::openAddEmployeeDialog);
...
void EmployeeDetailsWidget::openAddEmployeeDialog()
{
    LOG_DEBUG("openAddEmployeeDialog clicked");
    DialogFactory::registerDialogs();
    auto dlg = DialogFactory::create("add_employee");
    auto EmployeeDialog = dynamic_cast<AddEmployeeDialog *>(dlg);
    if (EmployeeDialog && EmployeeDialog->exec() == QDialog::Accepted)
        LOG_DEBUG("Employee Added");
    // m_Employee = EmployeeDialog->getEmployeeData();
};
